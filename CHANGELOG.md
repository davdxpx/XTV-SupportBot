...
### Changed
- **Slimmed the environment surface:** `.env.example` and the env reference now list only the required boot/secret/infra variables. All operational knobs (SLA, auto-close, topic auto-delete, cooldown, broadcast tuning, branding, UI mode, default language, feature flags) are configured in the admin console → Settings and are no longer part of the setup checklist. Setting one in env still works as a boot default (back-compat), and `Settings` documents the split.
