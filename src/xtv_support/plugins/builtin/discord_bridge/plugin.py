"""Discord-bridge plugin — mirrors ticket events into a Discord channel.

Uses the ``DISCORD_WEBHOOK_URL`` environment variable as the target.
Every supported domain event is rendered via
...
_log = get_logger("plugin.discord_bridge")


class Plugin(_Base):
    name = "discord_bridge"
    version = "0.1.0"
    feature_flag = "DISCORD_BRIDGE"
    description = "Mirror ticket events into a Discord channel via an incoming webhook."
...
async def _post(url: str, payload: dict[str, Any]) -> None:
    """POST a payload to ``url`` with best-effort error handling.

    Uses ``httpx`` when available (the ``api`` extra already pulls it in)
    and silently skips when httpx isn't installed so Discord/Slack
    bridges don't crash in CI images without the HTTP client.
    """
    try:
        import httpx  # type: ignore[import-untyped]
    except ModuleNotFoundError:
        _log.debug("discord_bridge.httpx_missing")
        return
    try:
        ...
        _log.warning("discord_bridge.post_error", error=str(exc))
