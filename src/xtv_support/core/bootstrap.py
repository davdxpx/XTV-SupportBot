...
from xtv_support.infrastructure.db.external_directory_config import (
    get_config as get_external_dir_config,
)
...
async def build_context(client: Client) -> HandlerContext:
    ...
    if ext_dir_config and ext_dir_config.enabled:
        try:
            ...
            if secret_uri:
                real_provider = ExternalDirectoryProvider(
                    ext_dir_config, secret_uri, ExternalConnectionManager()
                )
            ...
            log.error(
                "external_directory.provider_failed",
                error=str(e),
